...
for k, v in pairs(lib.load('data.clothes')) do
	local data = {}
	data.name = k
	data.label = v.label or k
	data.description = ("%s %s %s"):format(v.c, v.d, v.t)
	data.weight = v.weight or 100
	data.clothing = true
	data.stack = false
	data.type = v.type
	data.component = v.c
	data.drawable = v.d
	data.texture = v.t
	data.image = ("https://supabase.lorraxs.dev/storage/v1/object/public/%s/%s.png"):format("items", k)
	if isServer then
