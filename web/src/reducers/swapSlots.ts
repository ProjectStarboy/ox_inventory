...
> = (state, action) => {
  ...
  if (fromType === InventoryType.PLAYER && toType === InventoryType.CLOTHING) {
    [targetInventory.items[toSlot.slot], sourceInventory.items[fromSlot.slot - 1]] = [
      {
      ...
      {
        ...targetInventory.items[toSlot.slot],
        slot: fromSlot.slot,
  ...
  if (fromType === InventoryType.CLOTHING && toType === InventoryType.PLAYER) {
    [targetInventory.items[toSlot.slot - 1], sourceInventory.items[fromSlot.slot]] = [
      {
